...
fn parse_text_and_version(s: &str, args: &Args) -> Result<(String, Version), String> {
    match args.raw_policy_version {
        ...
        None => {
            let (policy_text, version) = extract_policy(s)
                .map_err(|e| format!("{}", e))?;
    ...
    match args.mode {
        Mode::Document => {
            match args.check_mode {
                true => {
                    PolicyParser::parse(Rule::file, &policy_text)
                        .map_err(|e| format!("{}", e))?;
        ...
        Mode::Expression => {
            ...
            let token = pairs.next()
                .ok_or_else(|| String::from("No tokens"))?;
...
pub fn main() -> ExitCode {
    ...
    let mut file: Box<dyn Read> = if let Some(ref file_name) = args.file {
        let f = OpenOptions::new().read(true).open(file_name)
