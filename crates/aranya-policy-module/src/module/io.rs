...
use zerocopy::{CastError, FromBytes as _, IntoBytes as _, little_endian};

use crate::{Module, ModuleData, Version};

const MODULE_MAGIC: [u8; 8] = [0x50, 0x4d, 0x4f, 0x44, 0xcb, 0xb3, 0xcc, 0xbe];
pub(crate) const CHECKSUM_HASH_LEN: usize = OutputSize::<Sha256>::USIZE;
...
// `zerocopy`'s errors are elided because they contain a slice reference I don't want to carry
// around in the error type.
impl From<CastError<&[u8], Header>> for ModuleIoError {
    fn from(_: CastError<&[u8], Header>) -> Self {
        Self::ZeroCopy
    }
}
...
impl Module {
    ...
    pub fn write_to_slice(&self, s: &mut [u8]) -> Result<usize, ModuleIoError> {
        ...
        // It should be exceedingly unlikely we can overflow a `usize` here, so I'm just using
        // `saturating_add()`. Even if the impossible happens and this reaches overflow, saturation
        // will keep it at `usize::MAX`, where the conversion to `u32` will fail later.
        let header_data_len = compressed_size.saturating_add(Header::LEN);
        let header_data_checksum_len = header_data_len.saturating_add(CHECKSUM_HASH_LEN);
        if s.len() < header_data_checksum_len {
            return Err(ModuleIoError::SliceTooSmall);
        }

        let h = Header {
            version: self.version().to_u32().into(),
            size: u32::try_from(header_data_checksum_len)?.into(),
            uncompressed_size: u32::try_from(module_data.len())?.into(),
            ..Default::default()
        };
        h.write_to_prefix(s)
            .map_err(|_| ModuleIoError::SliceTooSmall)?;

        let hash: [u8; CHECKSUM_HASH_LEN] = Sha256::digest(&s[..header_data_len]).into();
    ...
    pub fn read_from_slice(s: &[u8]) -> Result<Self, ModuleIoError> {
        let (h, _) = Header::ref_from_prefix(s)?;
        ...
        let computed_checksum: [u8; CHECKSUM_HASH_LEN] =
            Sha256::digest(&s[..header_data_len]).into();
        if checksum != computed_checksum {
            return Err(ModuleIoError::ChecksumMismatch);
        }
        let compressed_data = &s[Header::LEN..header_data_len];
        let uncompressed_size = u32::from(h.uncompressed_size).try_into()?;
        let data = zrip::decompress_with_limit(compressed_data, uncompressed_size)?;
        let module_data = match Version::try_from_u32(h.version.into()) {
...
mod tests {
    ...
    use crate::{
        Module, ModuleData, ModuleIoError, ModuleV1,
        module::io::{CHECKSUM_HASH_LEN, Header},
        v1,
    };
    ...
    fn recompute_checksum(s: &mut [u8]) {
        let hash_boundary = s.len().saturating_sub(CHECKSUM_HASH_LEN);
        let new_checksum: [u8; CHECKSUM_HASH_LEN] = Sha256::digest(&s[..hash_boundary]).into();
    ...
    fn bad_checksum() {
        ...
        let before_checksum = n.saturating_sub(CHECKSUM_HASH_LEN);
    ...
    fn bad_postcard() {
        ...
        let new_total_size = Header::LEN
            ...
            .saturating_add(CHECKSUM_HASH_LEN);
        let new_header = Header {
            version: 1.into(),
            size: u32::try_from(new_total_size).unwrap().into(),
            uncompressed_size: u32::try_from(bad_data.len()).unwrap().into(),
            ..Default::default()
        };
        new_header
