...
use aps::{self, ApsState, AranyaState, ChannelId, Client, Label, NodeId};
use crypto::{
    aps::{
...
use crate::{
    ffi::{ApsBidiChannel, ApsUniChannel, Ffi},
    handler::{
...
pub trait TestImpl: Sized {
    ...
    /// Configures a user for the test.
    fn new() -> User<Self>;
}

/// A test user.
pub struct User<T: TestImpl> {
    eng: T::Engine,
    /// The user's ID.
    user_id: UserId,
    /// The user's encryption key ID.
    enc_key_id: EncryptionKeyId,
    /// The user's encoded `EncryptionPublicKey`.
    enc_pk: Vec<u8>,
    /// Makes FFI calls.
    ffi: Ffi<T::Store>,
    /// Handles effects.
    handler: Handler<T::Store>,
    /// APS encryption/decryption client.
    aps_client: Client<T::Aps>,
    /// Aranya's view of the shared state.
    aps_state: T::Aranya,
    /// Maps `UserId`s to `NodeId`s.
    mapping: IndexSet<UserId, BuildHasherDefault<SipHasher13>>,
}
...
impl<T: TestImpl> User<T> {
    ...
    pub fn new(mut eng: T::Engine, aps: T::Aps, aranya: T::Aranya, mut store: T::Store) -> Self {
        ...
        Self {
            ...
            handler: Handler::new(user_id, store),
            aps_client: Client::new(aps),
            aps_state: aranya,
    ...
    fn test_bad_label(sealer: &mut Self, opener: &mut Self, label: Label) {
        ...
        assert_eq!(err, aps::Error::NotFound(id));
    }
...
/// use aps_util::testing::{test_all, MemStore, TestImpl, User};
/// use crypto::{
    ...
    ): for<'a> Transform<(&'a BidiChannel<'a, T::Engine>, BidiPeerEncap<T::Engine>)>,
{
    ...
    let ApsBidiChannel { peer_encap, key_id } = author
        .ffi
        .create_bidi_channel(
            &ctx,
            &mut author.eng,
    ...
    User::test_roundtrip(&mut author, &mut peer, label);
    User::test_roundtrip(&mut peer, &mut author, label);

    User::test_bad_label(&mut author, &mut peer, Label::new(123));
        ...
        for<'a> Transform<(&'a UniChannel<'a, T::Engine>, UniPeerEncap<T::Engine>)>,
{
    ...
    // This is called via FFI.
    let ApsUniChannel { peer_encap, key_id } = author
        .ffi
        .create_uni_channel(
            &ctx,
            &mut author.eng,
        ...
        let keys = author
            ...
            .uni_channel_created(
                ...
                &UniChannelCreated {
                    parent_cmd_id,
                    author_id: author.user_id,
                    seal_id: author.user_id,
                    open_id: peer.user_id,
                    author_enc_key_id: author.enc_key_id,
                    peer_enc_pk: &peer.enc_pk,
                    label,
                    key_id: key_id.into(),
    ...
    User::test_roundtrip(&mut author, &mut peer, label);
    User::test_bad_label(&mut author, &mut peer, Label::new(123));
    User::test_wrong_direction(&mut peer, &mut author, label);
}
        ...
        let keys = author
            ...
            .uni_channel_created(
                ...
                &UniChannelCreated {
                    parent_cmd_id,
                    author_id: author.user_id,
                    seal_id: peer.user_id,
                    open_id: author.user_id,
                    author_enc_key_id: author.enc_key_id,
                    peer_enc_pk: &peer.enc_pk,
                    label,
                    key_id: key_id.into(),
