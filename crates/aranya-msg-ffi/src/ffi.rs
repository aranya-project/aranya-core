...
use aranya_crypto::{
    ...
    EncryptionPublicKey, GroupKey, Id, KeyStore, KeyStoreExt, SigningKey, VerifyingKey,
};
use aranya_policy_vm::{ffi::ffi, CommandContext, Text};

use crate::error::{AllocError, Error, ErrorKind, KeyNotFound, WrongContext};

/// An [`FfiModule`][aranya_policy_vm::ffi::FfiModule] for message encryption.
///
...
"#)]
    pub(crate) fn encrypt_message<E: Engine>(
        ...
        let CommandContext::Action(ctx) = ctx else {
            return Err(WrongContext("`msg::encrypt_message` called outside of an action").into());
        };
...
"#)]
    pub(crate) fn decrypt_message<E: Engine>(
        ...
        let CommandContext::Policy(ctx) = ctx else {
            return Err(
                WrongContext("`msg::decrypt_message` called outside of a `policy` block").into(),
            );
        };
        let group_key: GroupKey<E::CS> = {
            let wrapped = postcard::from_bytes(&wrapped_group_key)?;
