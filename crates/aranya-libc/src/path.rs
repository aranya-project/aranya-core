...
#[repr(transparent)]
pub struct Path(
    /// Even though (almost) all of the public APIs require
    /// inputs to be null terminated, this slice is NOT
    /// necessarily null terminated. This is because we want to
    /// interop with `std::path::Path` and `std::path::PathBuf`
    /// which only expose `std::ffi::OsStr` and
    /// `std::ffi::OsString` which do not contain a null byte.
    ///
...
impl Path {
    ...
    fn try_new<S: AsRef<[u8]> + ?Sized>(path: &S) -> Result<&Self, &Self> {
        ...
        memchr::memchr(0, path)
            ...
            .ok_or_else(|| Self::from_raw_bytes(path))
    }
