...
use aranya_quic_channels::{self, AqcState, AranyaState, ChannelId, Client, Label, NodeId};
...
pub trait TestImpl: Sized {
    ...
    type Engine: Engine;
    /// The [`AqcState`] to use.
    type Aqc: AqcState<CipherSuite = <Self::Engine as Engine>::CS>;
...
pub struct Device<T: TestImpl> {
    ...
    /// AQC encryption/decryption client.
    aqc_client: Client<T::Aqc>,
...
impl<T: TestImpl> Device<T> {
    ...
    pub fn new(mut eng: T::Engine, aqc: T::Aqc, aranya: T::Aranya, mut store: T::Store) -> Self {
    ...
    fn test_roundtrip(sealer: &mut Self, opener: &mut Self, label: Label) {
        ...
        let ciphertext = {
            ...
            let mut dst = vec![0u8; GOLDEN.len() + Client::<T::Aqc>::OVERHEAD];
            sealer
                .aqc_client
        ...
        let (plaintext, got_label, got_seq) = {
            ...
            let mut dst = vec![0u8; ciphertext.len() - Client::<T::Aqc>::OVERHEAD];
    ...
    fn test_bad_label(sealer: &mut Self, opener: &mut Self, label: Label) {
        ...
        assert_eq!(err, aranya_quic_channels::Error::NotFound(id));
    }
...
///     type Aqc = State<DefaultCipherSuite>;
