...
use aranya_crypto::{
    self,
    // TODO: depends on aranya_crypto::aqc implementation
...
"#)]
    pub(crate) fn create_bidi_channel<E: Engine>(
        ...
        let label: aranya_quic_channels::Label = label.into();
...
pub(crate) enum FfiError {
    ...
    /// ACQ failed.
    #[error("ACQ error: {0}")]
    Aqc(#[from] aranya_quic_channels::Error),
...
impl From<Label> for aranya_quic_channels::Label {
    fn from(label: Label) -> Self {
        label.0.into()
    }
}

impl From<aranya_quic_channels::Label> for Label {
    fn from(label: aranya_quic_channels::Label) -> Self {
        Self(label.to_u32())
    }
}
